...
rownames(lost_matrix) <- c("Sequenced", "Adapter sequences","Not aligned", "Quality-filtered",
                           "Size- and Blacklist-filtered", "Nucleosomes")
...
rel_frame$Type <- factor(rel_frame$Type, levels = c("Not aligned", "Adapter sequences", "Quality-filtered",  
                                                    "Size- and Blacklist-filtered", "Nucleosomes"))
rel_frame$Counts <- as.numeric(rel_frame$Counts)
r <- ggplot(rel_frame, aes(x = Sample, y = Counts, fill = Type)) + 
  geom_bar(stat = "identity") +
  scale_fill_manual(values=c( "gray20","gray40","gray60", "gray80", "coral3")) + 
  xlab("") +
...
plot_frame$Type <- factor(plot_frame$Type, levels = c("Not aligned", "Adapter sequences", "Quality-filtered",  
                                                      "Size- and Blacklist-filtered", "Nucleosomes"))
